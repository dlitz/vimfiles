" File: closetag.vim Summary: Functions and mappings to close open HTML/XML
" tags Uses: <C-_> -- close matching open tag Author: Steven Mueller
" <diffusor@ugcs.caltech.edu> Last Modified: Fri Jun 08 01:22:06 PDT 2001
" Version: 0.9 TODO - make matching work for all comments -- kinda works now,
" but needs syn sync minlines to be very long -- Only check whether in syntax
" in the beginning, then store comment tags in the tagstacks to determine
" whether to move into or out of comment mode TODO - The new normal mode
" mapping clears recent messages with its <ESC>, and it doesn't fix the
" null-undo issue for vim 5.7 anyway.
"
" Description: This script eases redundant typing when writing html or xml
" files (even if you're very good with ctrl-p and ctrl-n  :).  Hitting ctrl-_
...
" out of sync, or really slow if syn sync minlines is large.  Set the
" b:closetag_disable_synID variable to disable this feature if you have really
" big chunks of comment in your code and closing tags is too slow.
" 
...
" problem.
"
" Install: To use, place this file in your standard vim scripts directory, and
" source it while editing the file you wish to close tags in.  If the filetype
" is not set or the file is some sort of template with embedded HTML, you may
" force HTML style tag matching by first defining the b:closetag_html_style
" buffer variable.  Otherwise, the default is XML style tag matching.
"
" Example: :let b:closetag_html_style=1 :source ~/.vim/scripts/closetag.vim
"
" For greater convenience, load this script in an autocommand: :au Filetype
" html,xml,xsl source ~/.vim/scripts/closetag.vim
"
...
" seperated list of element names that should be ignored while finding
" matching closetags.  Checking is done according to the current setting of
" the ignorecase option.
"
" b:closetag_html_style   Define this (as with let b:closetag_html_style=1)
" and source the script again to set the unaryTagsStack to its default value
" for html.
"
" b:closetag_disable_synID  Define this to disable comment checking if tag
" closing is too slow.
"
" Changelog: June 07, 2001 Thursday * Added comment handling.  Currently
" relies on synID, so if syn sync minlines is small, the chance for failure is
" high, but if minlines is large, tagclosing becomes rather slow...
"
"   * Changed normal mode closetag mapping to use <C-R> in insert mode rather
"   than p in normal mode.  This has 2 implications: - Tag closing no longer
"   clobbers the unnamed register - When tag closing fails or finds no match,
"   no longer adds to the undo buffer for recent vim 6.0 development versions.
...
if !exists("b:unaryTagsStack") || exists("b:closetag_html_style")
    if &filetype == "html" || &filetype == "dtml" || exists("b:closetag_html_style")
	let b:unaryTagsStack="Area Base Br DD DT HR Img Input LI Link Meta P Param dtml-call dtml-var dtml-else"
